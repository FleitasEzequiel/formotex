...
// import { DATABASE } from "../const/config";
...
const db =
{
   ...
   "get":async(schema:string) : Promise<WithId<Document>[]>=>(await client().db(DATABASE).collection(schema).find({}).toArray().finally(()=>client().close())),
